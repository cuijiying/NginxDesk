...
const titles={overview:'服务概览',config:'配置文件',sites:'新建站点',logs:'运行日志',backups:'配置备份'};
function output(text,error=false){$('output').textContent=(error?'操作失败：\n':'')+text;$('operation-time').textContent=new Date().toLocaleTimeString();}
async function task(fn){if(busy)return;busy=true;document.querySelectorAll('button').forEach(b=>b.disabled=true);try{await fn();}catch(e){output(e.message,true);}finally{busy=false;document.querySelectorAll('button').forEach(b=>b.disabled=false);}}
function markDirty(value){dirty=value;$('dirty').textContent=value?'有未保存的修改':'已保存';}
async function state(){const s=await window.desk.state();$('status').textContent=s.running?'● 运行中':'○ 已停止';$('pid').textContent=s.running?`主进程 PID ${s.pid}`:'随时可以启动服务';$('version').textContent=s.version.replace('nginx version: ','');$('file-count').textContent=String(s.files.length).padStart(2,'0');$('root').textContent=s.root;const names=[...s.files];if(currentFile&&!names.includes(currentFile))names.push(currentFile);$('files').replaceChildren(...names.map(name=>{const o=document.createElement('option');o.value=name;o.textContent=name;return o;}));if(currentFile)$('files').value=currentFile;return s;}
...
async function show(page){activePage=page;document.querySelectorAll('.page').forEach(p=>p.classList.toggle('active',p.id===page));document.querySelectorAll('nav button').forEach(b=>b.classList.toggle('active',b.dataset.page===page));$('page-title').textContent=titles[page];if(page==='logs')await logs();if(page==='backups')await backups();}
...
$('refresh-log').onclick=()=>task(logs);$('log-type').onchange=()=>task(logs);$('refresh-backups').onclick=()=>task(backups);
