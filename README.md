...
配置目录：`%APPDATA%/nginx-desk/runtime`（实际位置以界面为准）。软件升级与卸载默认保留用户数据。主配置必须保留 `pid logs/nginx.pid;`，日志页读取 `logs/error.log` 与 `logs/access.log`。可编辑完整 nginx 配置来设置 TLS、上游负载均衡、缓存等；这些高级项目目前没有专用表单。不要在运行期间从外部修改 PID 路径。
