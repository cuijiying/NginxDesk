...
**Nginx Desk** 是一个跑在 Windows、macOS 与 Linux 上的桌面程序：它自带一份官方 nginx，帮当前用户在本机启动、停止、改配置、看日志、切换版本。也可以连接到本机或远程已经存在的 nginx。用户不必先装 nginx 才能开始用托管实例，也不必配置环境变量。

它**不是**：

- 系统服务（关机后不会自动起来，也没有开机自启）
- 证书申请 / 负载均衡专用面板（这些可以手写进配置，但没有独立表单）
- 没有用户确认就把机器上其它 nginx 停掉的工具（附加实例必须在「连接实例」里明确添加）
...
| 用户动作 | 背后实际发生的事 |
|----------|------------------|
| 启动服务 | `nginx -t` 通过后，以工作目录为 prefix 拉起本机引擎 |
| 优雅停止 | `nginx -s quit`，等工作进程处理完连接 |
| 重载配置 | 先 `-t`，再 `-s reload` |
| 校验并保存 | 写盘 → `-t`；失败则回滚，并留下上一份备份 |
| 新建站点 | 按表单生成 `server { ... }`，预览后再保存 |
| 引擎版本 | Windows 下官方 zip，抽出 `nginx.exe`；macOS/Linux 下官方源码并编译，再替换当前引擎。可删除非当前缓存 |

## 1.3 给完全新手的 nginx 概念

nginx 是一个 Web 服务器 / 反向代理。你只需要先记住四个概念：

1. **可执行文件**：Windows 是 `nginx.exe`，macOS / Linux 是 `nginx`。真正干活的程序。
...
4. **信号**：用 `nginx -s quit|reload|reopen` 控制已在运行的实例。它靠 **PID 文件** 找到进程。本机托管实例强制 PID 文件必须是 `logs/nginx.pid`；附加的已有实例使用其配置里的 pid 路径。
...
macOS / Linux 使用的是官方源码编译出的 Unix nginx，行为更接近常规服务器部署。
...
┌──────────────────────────────────────────────┐
│  <userData>/runtime/                         │
│    nginx 或 nginx.exe  ← 当前启用的引擎副本  │
│    conf/               ← 主配置 + 站点       │
│    logs/               ← PID、错误、访问日志 │
│    html/ backups/ temp/                      │
│  <userData>/engines/                         │
│    1.31.6/nginx(.exe)  ← 已下载/编译版本缓存 │
│    active              ← 用户钉住的版本号    │
└──────────────────────────────────────────────┘

不会自动去碰
  系统包管理器安装的 nginx、其他用户目录里的 nginx
  （除非用户在「连接实例」里明确添加并切换过去）
```

`userData` 由 Electron 决定：
...
状态检测时，不只看 PID 数字是否存在，还会核对「这个 PID 的可执行文件路径」是不是当前连接指定的引擎。路径对不上就当成「未运行」，避免误操作未连接的进程。
...
| `connections` | 连接实例 | 本机托管 / 本机已有 / 远程 SSH；探测、测试、保存、切换 |
| `engines` | 引擎版本 | 拉 nginx.org 列表，安装、启用或删除指定官方版本（仅托管实例） |
...
- **安装包**：`npm run dist` 打出当前操作系统的安装包。用户装好后，nginx 在应用资源目录的 `nginx`，用户配置在 `userData`。
...
- 打包进去的 nginx：保留官方许可证，安装后在 `resources/nginx/docs/LICENSE`（macOS 为应用包内对应路径）。
