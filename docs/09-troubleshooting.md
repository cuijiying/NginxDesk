...
| 来源 | 在哪 | 什么问题 |
|------|------|----------|
| Electron 终端 | 运行 `npm start` 的 PowerShell | 主进程抛错、IPC、init 失败 |
...
仍失败：看脚本是哈希不匹配还是网络下载失败。公司代理需能为 `nginx.org` 走 HTTPS。

安装包用户若看到类似错误：`resources/nginx` 缺失或被杀毒隔离。到安装目录 `resources\nginx\nginx.exe` 确认文件在。
...
- PID 文件里的进程，其 `ExecutablePath` 不是 `runtime\nginx.exe`（例如用户从 `resources\nginx` 手动启动了一份）
...
- 保存成功只保证磁盘和 `-t`，**必须再点重载**
- 浏览器缓存：用无痕或强制刷新
- Windows nginx 的 worker 有时对极快的 reload 反应慢；集成测试会轮询数秒。可看 `error.log` 是否有 reload 报错
...
若官方页面改版导致解析失败：`parseWindowsVersions` 会抛「未能解析」。用测试里的 HTML 夹具对照真实页面结构，更新解析再发版。

## 9.7 安装 / 切换版本失败

| 提示 | 含义 |
|------|------|
| 请先停止 nginx | 运行中无法覆盖 `runtime\nginx.exe` |
| 无效的 nginx 版本号 | 不是 `x.y.z` |
| 不在官方 Windows 发行列表 | 防任意 URL 下载 |
| 安装包过大 / 不是有效 zip | 下载内容不像官方 zip |
| 安装包结构异常 | 解压后没有 `nginx-<ver>/nginx.exe` 或路径试图逃出临时目录 |
| 版本切换异常，当前为… | 拷贝后 `-v` 对不上，暂停使用并检查 engines 目录 |

`tar.exe` 不存在时解压会失败。Win10 早期或精简版需确认 `where tar`。
...
2. 到 `%TEMP%\nginx desk test *` 最新目录打开 `logs/error.log`
3. 确认没有杀毒锁定 `nginx.exe`
4. 确认没有手动在同一临时目录留着正在运行的 nginx（`finally` 会尝试 quit）
5. 非 Windows 上集成段会 skip，只跑纯函数——不要以为「测试全绿」等于 nginx 行为在 Linux CI 上被验证了
...
- extraResources 路径 `from: vendor/nginx` 必须是「目录内直接是 nginx.exe」。prepare 脚本的 `Move-Item` 失败时可能变成 `vendor/nginx/nginx-1.31.6/nginx.exe`，运行时 `bundled` 就找不到 exe

## 9.11 关闭窗口卡死或退不出

`close` 处理是异步的。若 `status()` 里 PowerShell 挂起（CIM 超时 10s），对话框会晚出现。连续点关闭可能排队。等提示出现；必要时任务管理器结束 **Nginx Desk**（不是随便结束所有 nginx——你可能选过「保持运行」）。

若选了停止但连接一直不结束：Windows nginx 的 `quit` 会等 worker。页脚可能已显示「正在等待连接结束」。过久则看是否有长连接/上传未完成。
